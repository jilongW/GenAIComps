...
from comps.finetuning.src.clip.finetune_config import FinetuneConfig, FineTuningParams
...
class OpeaFinetuning(OpeaComponent):
    ...
    def create_finetuning_jobs(self, request: FineTuningParams, background_tasks: BackgroundTasks):
        ...
        if finetune_config.General.tool == "clip":
            print(1111111111)
            ray_job_id = ray_client.submit_job(
                # Entrypoint shell command to execute
                entrypoint=f"cd clip/CLIP_LLama_Factory/src/llamafactory/CLIP_Finetune && export DATA=/home/data/jlwang/data && bash scripts/CLIP_finetune/clip_adapter_hf.sh caltech101 vit_b16 0 XPU >> test.log 2>&1",
            )
