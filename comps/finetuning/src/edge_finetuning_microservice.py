...
from comps.finetuning.src.clip.finetune_config import FineTuningParams
from comps.finetuning.src.clip.native import OpeaFinetuning, upload_file
from comps.finetuning.src.edge_finetuning_loader import EdgeFinetuningLoader

logger = CustomLogger("edge_finetuning_microservice")

finetuning_component_name = os.getenv("FINETUNING_COMPONENT_NAME", "OPEA_FINETUNING")
# Initialize OpeaComponentLoader
loader = EdgeFinetuningLoader(
    finetuning_component_name,
    description=f"OPEA FINETUNING Component: {finetuning_component_name}",
)


@register_microservice(name="opea_service@edge_finetuning", endpoint="/v1/edge_fine_tuning/jobs", host="0.0.0.0", port=8015)
...
@register_microservice(
    name="opea_service@edge_finetuning", endpoint="/v1/edge_fine_tuning/jobs", host="0.0.0.0", port=8015, methods=["GET"]
)
...
@register_microservice(
    name="opea_service@edge_finetuning", endpoint="/v1/edge_fine_tuning/jobs/retrieve", host="0.0.0.0", port=8015
)
...
@register_microservice(
    name="opea_service@edge_finetuning", endpoint="/v1/edge_fine_tuning/jobs/cancel", host="0.0.0.0", port=8015
)
...
@register_microservice(
    name="opea_service@edge_finetuning",
    endpoint="/v1/files",
    host="0.0.0.0",
    port=8015,
)
...
@register_microservice(
    name="opea_service@edge_finetuning", endpoint="/v1/finetune/list_checkpoints", host="0.0.0.0", port=8015
)
...
if __name__ == "__main__":
    opea_microservices["opea_service@edge_finetuning"].start()
